...
# -----------------------------------------
# FORCE RELEASE HANDLES + DELETE FOLDER
# -----------------------------------------

# Extra wait for MSI / service cleanup
Start-Sleep -Seconds 5

# Kill again (Windows is lazy)
Get-Process -ErrorAction SilentlyContinue | Where-Object {
    $_.ProcessName -like "*wazuh*" -or $_.ProcessName -like "*ossec*" -or
    $_.ProcessName -like "*msiexec*"
} | Stop-Process -Force

# Retry delete (up to 5 times)
$retries = 5
while ($retries -gt 0 -and (Test-Path $InstallPath)) {
    try {
        Remove-Item $InstallPath -Recurse -Force -ErrorAction Stop
    } catch {
        Write-Host "Folder still locked, retrying..."
...
if (Test-Path $InstallPath) {
    Write-Warning "ossec-agent folder could not be deleted (locked). Reboot may be required."
} else {
    Write-Host "ossec-agent folder deleted successfully"
}
